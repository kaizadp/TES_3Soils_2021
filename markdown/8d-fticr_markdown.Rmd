...
```{r vk_domains, fig.height=7,fig.width=13}
pore_long %>% 
...
gg_domains+gg_nosc + 
  ...
  theme(plot.tag = element_text(size = 13, face = "bold", hjust = 1, margin = margin(0,0,0, 10)))

```

# FTICR pores - PCA figures
```{r pores_pca, fig.height=7,fig.width=12, dpi=300}
# pore_raw = read.csv(FTICR_PORE_RAW_LONG)# <- "fticr/fticr_pore_raw_longform.csv"
...
ggbp_fine = 
  ...
  scale_fill_manual(values = c("#fde725ff", "#35b779ff", "#443a83ff", "#EBEBEB"))+
  ggtitle("50 kPa")+
  theme_kp()+
  theme(legend.position = "right",
...
```{r unique_site, fig.width=8, fig.height=5, dpi=300}
...
vk2 =
  ...
  scale_color_manual(values = c("#fde725ff", "#35b779ff", "#443a83ff"))+
  labs(title = "B: treatment incubations") + 
...
```{r combined_treatments, fig.height=10, fig.width=9, dpi=300}
library(patchwork)
combined = vk1/vk2 + plot_layout(heights = c(1,2), tag_level = "new")+
  #plot_annotation(tag_levels = list(c("A: time-zero", "B: incubations"))) & 
  #theme(plot.tag = element_text(size = 13, face = "bold", hjust = 1, margin = margin(0,0,0, 10)))+
  NULL
...
vk1 + facet_grid(tension~site) + labs(title = "Time Zero")
```
...
```{r vk_unique, fig.width=10, fig.height=7, dpi=300}
